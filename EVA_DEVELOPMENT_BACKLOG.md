...
| 31 | Video-gen cost research — cheapest model that still holds quality | DONE ✅ (research) | Compared hosted APIs + open-source options for EVA's video agent (2026 pricing/quality). **Hosted API winner:** [fal.ai](https://fal.ai/learn/tools/ai-video-generators) as the aggregator — consistently 30-80% cheaper than Replicate ([TeamDay.ai comparison](https://www.teamday.ai/blog/fal-ai-vs-replicate-comparison)); cheapest-yet-solid models on it: LTX-2 Fast ($0.04/s, 1080p) and Wan 2.5/2.6 ($0.05/s); Seedance 2.0 Fast ($0.09/s, 1080p) called the cheapest "production-quality" tier by [Atlas Cloud](https://www.atlascloud.ai/blog/guides/cheapest-ai-video-generation-api-2026). **Open-source self-host winner (best cheap-to-run + quality):** [Lightricks/LTX-Video (LTX-2)](https://github.com/Lightricks/LTX-Video) — Apache 2.0, real-time generation, runs on ~12GB VRAM, native synced 4K video+audio in one pass; drag-and-drop workflow via [Lightricks/ComfyUI-LTXVideo](https://github.com/Lightricks/ComfyUI-LTXVideo). Runner-ups: [Wan-Video/Wan2.2](https://github.com/Wan-Video/Wan2.2) (Apache 2.0, best quality-per-VRAM, multilingual, text+image-to-video) and [Tencent-Hunyuan/HunyuanVideo-1.5](https://github.com/Tencent-Hunyuan/HunyuanVideo-1.5) (Apache 2.0, best raw cinematic quality, needs 24GB+ VRAM). Self-hosted marginal cost ~$0.01-0.05/clip in electricity/GPU-rental vs $0.05-0.40/s on hosted APIs ([VideoToPrompt](https://www.videotoprompt.app/posts/open-source-ai-video-models-comparison-2026)). **Recommendation:** wire EVA's video agent to fal.ai (LTX-2 Fast / Wan 2.5) now for near-zero setup cost; self-host LTX-2 on a rented 24GB GPU (e.g. RunPod/Lambda, ~$0.30-0.50/hr) once volume justifies it. Within Computer's own built-in tool, `seedance_2_0` is already the best cheap pick (same cost tier as default `sora_2` but rated higher quality, longer durations, native music+lip-synced dialogue). |
