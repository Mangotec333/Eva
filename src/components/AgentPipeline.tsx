...
const STAGE_CONTENT: Record<Stage, React.ReactNode> = {
      ...
      <ol className="flex flex-col gap-1.5">
        {[
          ...
          <li key={i} className="flex items-start gap-2">
            ...
            <span className="font-mono text-xs text-gray-500">{step}</span>
          </li>
        ))}
      </ol>
    </div>
  ...
  STEPS: (
    <div className="flex flex-col gap-2">
      ...
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="font-mono text-xs text-gray-500 shrink-0">1 / 4</span>
      </div>
  ...
  'TOOL CALLS': (
    <div className="flex flex-col gap-2">
      ...
      <code className="font-mono text-xs text-green-400 bg-white border border-gray-200 rounded px-3 py-2 block leading-relaxed">
        browser_task(<br />
...
function RadarPulse() {
  return (
    <div className="flex flex-col items-center justify-center py-6 gap-4">
      ...
      <div className="font-mono text-xs text-gray-500 tracking-widest">EVA IDLE — awaiting task</div>
    </div>
  );
}
          ...
          <button
            ...
            className={`
              ...
              ${isActive
                ...
                : 'bg-gray-100 border border-gray-200 text-gray-500 hover:text-gray-500 hover:border-gray-300'}
            `}
          >
...
export function AgentPipeline() {
  ...
  const stagePill = activeStage ? (
    ...
    <span className="font-mono text-[10px] font-bold px-2 py-0.5 bg-gray-100 border border-gray-200 text-gray-500 rounded tracking-widest">
      IDLE
    </span>
  );

  return (
    <div className="bg-gray-50/60 border border-gray-200 rounded-xl overflow-hidden">
      {/* Header — always visible, click to toggle */}
      <button
        className="w-full flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50 hover:bg-gray-100/50 transition-colors"
        onClick={() => setCollapsed(c => !c)}
      >
        <div className="flex items-center gap-3">
          {/* Radar dot */}
          <div className={`w-2 h-2 rounded-full shrink-0 ${demoRunning ? 'bg-cyan-400 animate-pulse' : isIdle ? 'bg-gray-200' : 'bg-cyan-400 animate-pulse'}`} />
          <div className="text-left">
            <div className="font-mono text-xs font-bold text-gray-900 tracking-widest uppercase">
              EVA ACTIVE TASK
            </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1 bg-gray-100 border border-gray-200 text-gray-500 rounded font-mono text-[10px] font-bold hover:text-gray-800 hover:border-gray-500 transition-colors disabled:opacity-50"
          >
              ...
              <>
                ...
                <div className="flex gap-2 mt-2">
                  <input
                    ...
                    onKeyDown={e => {
                      if (e.key === 'Enter' && taskInput.trim() && !demoRunning) {
                        handleDemo();
                        setTaskInput('');
                      }
                    }}
                    placeholder="Give EVA a task..."
                    className="flex-1 bg-white border border-gray-200 rounded px-3 py-2 font-mono text-xs text-gray-800 placeholder:text-gray-400 focus:outline-none focus:border-cyan-500/60 transition-colors"
                  />
                  <button
                    onClick={() => {
                    ...
                    disabled={demoRunning || !taskInput.trim()}
                    className="flex items-center gap-1.5 px-3 py-2 bg-cyan-500/20 border border-cyan-400/50 text-cyan-300 rounded font-mono text-xs font-bold hover:bg-cyan-500/30 hover:border-cyan-400/80 transition-colors active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed"
                  >
