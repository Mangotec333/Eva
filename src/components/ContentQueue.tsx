...
function RejectModal({
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-sm bg-gray-50 border border-gray-200 rounded-xl shadow-2xl p-5 flex flex-col gap-4">
        <div className="font-mono text-sm font-bold text-red-400 tracking-wider">REJECT DRAFT</div>
        <textarea
          className="bg-white border border-gray-200 rounded p-3 text-sm text-gray-800 font-sans resize-none h-24 focus:outline-none focus:border-gray-500 placeholder:text-gray-500"
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={onCancel}
            className="flex-1 px-4 py-2 bg-gray-100 border border-gray-200 text-gray-500 rounded font-mono text-xs font-bold hover:bg-gray-200 transition-colors"
          >
            CANCEL
          </button>
        </div>
...
function DraftCard({
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col gap-3 hover:border-gray-200 transition-colors">
      {/* Type badge + platform */}
      <div className="flex items-center gap-2">
        <span className={`font-mono text-[10px] font-bold px-2 py-0.5 rounded ${badge.bg} ${badge.text} tracking-widest`}>
          {badge.label}
        </span>
        <span className="font-mono text-[10px] text-gray-500 tracking-widest">· ACTIVITY STREAM</span>
        <span className={`ml-auto font-mono text-[10px] font-semibold ${REACH_COLORS[draft.reach_estimate] ?? 'text-gray-500'}`}>
          ◆ REACH: {draft.reach_estimate}
        </span>
      </div>

      {/* Hook */}
      <div className="font-mono text-base font-bold text-gray-900 leading-snug">
        "{draft.hook}"
      </div>
      ...
      {editing ? (
        <div className="flex flex-col gap-2">
          <textarea
            className="bg-white border border-gray-200 rounded p-3 text-sm text-gray-800 font-sans resize-none h-40 focus:outline-none focus:border-cyan-500/60 placeholder:text-gray-500"
            value={editText}
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-gray-100 border border-gray-200 text-gray-500 rounded font-mono text-xs hover:bg-gray-200 transition-colors"
            >
              CANCEL
            </button>
          </div>
    ...
    <>
      ...
      <div className="bg-gray-50/60 border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 bg-gray-50">
          ...
          <div className="flex items-center gap-2">
            {/* Draft count badge */}
            <span className="font-mono text-[10px] font-bold px-2.5 py-1 bg-gray-100 border border-gray-200 rounded text-gray-500 tracking-widest">
        ...
        <div className="p-4">
          ...
          {!isOnline && !isLoading && (
            <div className="flex flex-col items-center justify-center py-8 gap-3 text-center">
              ...
              <div className="font-mono text-[10px] text-gray-400 bg-white border border-gray-200 rounded px-3 py-1.5">
          ...
          {isLoading && (
            <div className="flex items-center justify-center py-8 gap-2">
              ...
              <span className="font-mono text-xs text-gray-500">Checking content engine…</span>
            </div>
          )}
