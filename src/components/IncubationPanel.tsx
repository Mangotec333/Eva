...
const STATUS_CONFIG: Record<IncubationStatus, { label: string; bg: string; text: string; border: string }> = {
  ...
  archived:  { label: 'ARCHIVED',  bg: 'bg-gray-500/10',    text: 'text-gray-500',    border: 'border-gray-700/30'    },
};
...
function LogLearningInput({
  ...
  return (
    <div className="flex gap-2 mt-1">
      ...
      <button
        onClick={onCancel}
        className="px-2 py-1.5 bg-[#111] border border-[#1a1a1a] text-gray-500 rounded font-mono text-xs hover:text-gray-300 active:scale-95 transition-all cursor-pointer"
      >
        Cancel
      </button>
    </div>
  ...
  return (
    <div className="mt-2 p-3 bg-red-500/5 border border-red-500/20 rounded-lg flex flex-col gap-2">
      ...
      <input
        ...
        autoFocus
        className="bg-[#0a0a0a] border border-[#1a1a1a] rounded px-3 py-1.5 font-mono text-xs text-gray-300 placeholder:text-gray-700 focus:outline-none focus:border-red-500/40 transition-colors"
      />
      <div className="flex gap-2">
        ...
        <button
          onClick={onCancel}
          className="px-3 py-1.5 bg-[#111] border border-[#1a1a1a] text-gray-500 rounded font-mono text-xs hover:text-gray-300 active:scale-95 transition-all cursor-pointer"
        >
          Cancel
        </button>
      </div>
...
function IncubationCard({
  ...
  return (
    ...
    >
      ...
      {/* ── Hypothesis ── */}
      <p className="font-sans text-[11px] text-gray-600 italic leading-relaxed line-clamp-2">
        {inc.hypothesis}
      </p>
      ...
      <div className="flex items-center justify-between px-3 py-2 bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg">
        <div className="flex flex-col gap-0.5">
          <span className="font-mono text-[9px] text-gray-600 uppercase tracking-widest">Target</span>
          <span className="font-mono text-xs text-gray-300 font-semibold">{m.target}</span>
        </div>
        <div className="w-px h-6 bg-[#1a1a1a]" />
        <div className="flex flex-col gap-0.5 text-right">
          <span className="font-mono text-[9px] text-gray-600 uppercase tracking-widest">Current</span>
      ...
      <div className="flex flex-wrap gap-1.5">
        {inc.resources.map((r, i) => (
          <span
            key={i}
            className="px-2 py-0.5 bg-[#0a0a0a] border border-[#1a1a1a] rounded font-mono text-[9px] text-gray-600"
          >
            {r}
          </span>
        ))}
      ...
      {inc.learnings && inc.learnings.length > 0 && (
        <div className="flex flex-col gap-1.5 bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg px-2.5 py-2">
          <span className="font-mono text-[9px] text-gray-600 uppercase tracking-widest">Learnings</span>
          <ul className="flex flex-col gap-1">
      ...
      {isArchived && inc.reason && (
        <div className="px-3 py-2 bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg">
          <span className="font-mono text-[9px] text-gray-600 uppercase tracking-widest block mb-0.5">Reason</span>
      ...
      <div className="flex items-center justify-between gap-2 pt-1 border-t border-[#1a1a1a]">
        ...
        <div>
          {inc.decision === null ? (
            <span className="font-mono text-[10px] text-gray-700 uppercase tracking-wider">Pending Decision</span>
          ) : inc.decision === 'go' ? (
          ...
          <div className="flex items-center gap-1.5">
            ...
            {editing ? (
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 bg-[#0a0a0a] border border-[#1a1a1a] text-gray-500 rounded font-mono text-[9px] hover:text-gray-300 hover:border-[#2a2a2a] active:scale-95 transition-all cursor-pointer"
              >
    ...
    <div className="bg-[#111] border border-[#1a1a1a] rounded-xl overflow-hidden">
        ...
        <div className="px-4 pb-4 grid grid-cols-2 gap-3 border-t border-[#1a1a1a]" style={{ paddingTop: '12px' }}>
          {archived.map(inc => (
            ...
            >
              ...
              <span className="font-sans text-sm font-bold text-gray-400">{inc.name}</span>
              <span className="font-mono text-[10px] text-gray-600">{inc.tagline}</span>
              {inc.reason && (
              ...
              {inc.outcome && (
                <div className="px-2.5 py-2 bg-[#111] border border-[#1a1a1a] rounded">
                  <span className="font-mono text-[9px] text-gray-600 uppercase tracking-widest block mb-0.5">Outcome</span>
      ...
      {open && archived.length === 0 && (
        <div className="px-4 py-6 border-t border-[#1a1a1a] text-center">
          <span className="font-mono text-xs text-gray-700">No archived incubations yet.</span>
        </div>
      )}
    </div>
  );
      ...
      {active.length > 0 ? (
        ...
        <div className="flex items-center justify-center py-16 bg-[#111] border border-[#1a1a1a] rounded-xl">
          <div className="flex flex-col items-center gap-2">
            <span className="font-mono text-xs text-gray-600">No active incubations</span>
