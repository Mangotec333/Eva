...
function ActivityRow({ activity }: { activity: ContextActivity }) {
  ...
  return (
    <div className="flex items-start gap-2.5 py-2 border-b border-gray-200/50 last:border-0">
      {/* Icon / dot */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <span className="flex-shrink-0 font-mono text-[10px] text-gray-400 whitespace-nowrap">
  ...
  return (
    <div className="flex flex-col items-center justify-center py-8 gap-3">
      ...
      <div className="text-center">
        ...
        <div className="font-sans text-xs text-gray-500 mt-1 max-w-[220px] text-center">
          Start your morning sequence to bring the context engine online
        </div>
      </div>

      {/* Morning sequence hint */}
      <div className="w-full bg-gray-100/50 border border-gray-200/50 rounded p-3 mt-1">
        ...
        <div className="space-y-1">
          {[
            ...
            <div key={cmd} className="flex items-center gap-2">
              ...
              <code className="font-mono text-[10px] text-cyan-400/80 bg-gray-50/60 px-1.5 py-0.5 rounded">
                {cmd}
              </code>
            </div>
          ))}
...
export function ActivityFeed({ context, loading, status, lastUpdated, onRefresh }: ActivityFeedProps) {
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col gap-3 h-full">
      {/* Header */}
      ...
      {status === 'online' && context && (
        <div className="flex items-center gap-4 px-3 py-2 bg-gray-100/50 border border-gray-200/50 rounded">
          ...
          {sessions !== undefined && (
            <div className="flex items-center gap-1">
              <span className="font-sans text-[10px] text-gray-500">Sessions:</span>
              <span className="font-mono text-xs text-cyan-400 font-bold">{sessions}</span>
            </div>
          )}
          {energyLevel !== undefined && (
            <div className="flex items-center gap-1">
              <span className="font-sans text-[10px] text-gray-500">Energy:</span>
      ...
      <div className="flex-1 overflow-y-auto">
        {status === 'offline' ? (
          ...
          <div className="flex flex-col items-center justify-center py-8 gap-2">
            ...
            <span className="font-mono text-xs text-gray-500">No activity recorded today</span>
          </div>
        )}
      </div>
