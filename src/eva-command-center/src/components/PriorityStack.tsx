...
const TRACKS: Priority[] = [
  ...
  },
  {
    ...
    status: 'FLYWHEEL',
    description: '🎯 30-DAY SPRINT · Health/Wellness SaaS · $10K net/mo after debt service · HELOC $200K ready',
    metric: '$0 cashflow',
    flywheel: true,
  },
  {
...
const STATUS_CONFIG: Record<PriorityStatus, { bg: string; text: string; border: string; dot: string; label: string }> = {
  ...
  'BACKGROUND': {
    bg: 'bg-gray-200/20', text: 'text-gray-500', border: 'border-gray-200/40', dot: 'bg-gray-600',
    label: 'BACKGROUND',
  },
  'PAUSED': {
    ...
    <div
      className={`flex items-center gap-3 px-3 py-2 rounded border transition-colors
        ${isActive
          ...
          : isBackground
          ? 'bg-gray-100/20 border-gray-200/40 hover:bg-gray-100/40'
          : 'bg-gray-100/50 border-gray-200/50 hover:bg-gray-100'}
      `}
    >
      {/* Rank badge */}
      ...
      <Icon className={`flex-shrink-0 w-3.5 h-3.5 ${
        ...
        isBackground ? 'text-gray-500' : 'text-gray-500'}`}
      />

      {/* Name + Description */}
      <div className="flex-1 min-w-0">
        <div className={`font-mono text-xs font-semibold leading-none mb-0.5 truncate
          ${isBackground ? 'text-gray-500' : 'text-gray-800'}`}
        >
          {track.name}
          {/* Flywheel marker */}
          {track.flywheel && (
        ...
        <div className="font-sans text-xs text-gray-500 leading-snug truncate">
          {track.description}
        </div>
      </div>
...
export function PriorityStack() {
  ...
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        <div className="font-mono text-[10px] text-gray-500 tracking-widest uppercase px-1 mb-0.5">
