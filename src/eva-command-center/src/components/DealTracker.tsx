...
const STAGE_CONFIG: Record<string, { bg: string; text: string; border: string }> = {
  Tracking:       { bg: 'bg-gray-200/30',    text: 'text-gray-500',   border: 'border-gray-200/40' },
...
function MarketStatusBadge({ status }: { status: string }) {
  ...
  const cfg =
    s === 'available'
      ...
      : { bg: 'bg-gray-200/30', text: 'text-gray-500', border: 'border-gray-200/40', label: 'OFF MARKET' };
...
function SourceBadge({ source }: { source: string }) {
  ...
  const config =
    lower.includes('empire') || lower.includes('ef')
      ...
      : { bg: 'bg-gray-200/20', text: 'text-gray-500', border: 'border-gray-200/30' };
...
function OverallScoreBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-[80px]">
      ...
      <span className="font-mono text-xs text-gray-500 tabular-nums w-6 text-right">{score}</span>
    </div>
  );
}
...
function AdvanceStageModal({
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="w-full max-w-md bg-gray-50 border border-gray-200 rounded-xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
          <span className="font-mono text-sm font-bold text-cyan-400 tracking-wider">ADVANCE STAGE</span>
          <button onClick={onClose} className="p-1 text-gray-500 hover:text-gray-500 rounded transition-colors"><X className="w-4 h-4" /></button>
        </div>
        <div className="px-5 py-4 flex flex-col gap-4">
          <div className="flex items-center gap-3 font-mono text-sm">
            <StageBadge stage={deal.stage} />
            <ChevronRight className="w-4 h-4 text-gray-500" />
            {next ? <StageBadge stage={next} /> : <span className="text-gray-500">—</span>}
          </div>
          <div>
            <label className="font-mono text-xs text-gray-500 uppercase tracking-wide block mb-1.5">Reason *</label>
            <textarea
              className="w-full bg-white border border-gray-200 rounded px-3 py-2 font-mono text-sm text-gray-800 resize-none focus:outline-none focus:border-cyan-500/60 transition-colors"
              rows={3}
        ...
        <div className="px-5 py-4 border-t border-gray-200 flex justify-end gap-2">
          <button onClick={onClose} className="px-3 py-1.5 font-mono text-xs text-gray-500 hover:text-gray-500 border border-gray-200 rounded transition-colors">Cancel</button>
          <button
...
function ArchiveModal({
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="w-full max-w-md bg-gray-50 border border-red-900/50 rounded-xl shadow-2xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
        ...
        <div className="px-5 py-4 flex flex-col gap-4">
          <p className="font-mono text-xs text-gray-500">
            Archiving: <span className="text-gray-800 font-semibold">{deal.name}</span>
          </p>
          ...
          <div>
            ...
            <textarea
              className="w-full bg-white border border-gray-200 rounded px-3 py-2 font-mono text-sm text-gray-800 resize-none focus:outline-none focus:border-red-500/60 transition-colors"
              rows={3}
...
function HistoryModal({
  ...
  return (
    <ModalOverlay onClose={onClose}>
      <div className="w-full max-w-lg bg-gray-50 border border-gray-200 rounded-xl shadow-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200 shrink-0">
          <div className="flex items-center gap-2">
            <Clock className="w-4 h-4 text-cyan-400" />
            <span className="font-mono text-sm font-bold text-cyan-400 tracking-wider">DEAL HISTORY</span>
            <span className="font-mono text-xs text-gray-500">— {deal.name}</span>
          </div>
          <button onClick={onClose} className="p-1 text-gray-500 hover:text-gray-500 rounded transition-colors"><X className="w-4 h-4" /></button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-4 flex flex-col gap-3">
          {loading ? (
            <div className="flex flex-col gap-2">
              {[1,2,3].map(i => <div key={i} className="h-10 bg-gray-100/50 rounded animate-pulse" />)}
            </div>
          ) : history.length === 0 ? (
            <p className="font-mono text-sm text-gray-500 text-center py-6">No history recorded yet.</p>
          ) : (
            history.map(item => (
              <div key={item.id} className="flex flex-col gap-0.5 py-2.5 border-b border-gray-200 last:border-0">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-xs font-semibold text-gray-500 uppercase tracking-wide">{item.event_type}</span>
                  <span className="font-mono text-[10px] text-gray-500">{formatDate(item.created_at)}</span>
                </div>
                {(item.from_value || item.to_value) && (
                  <div className="flex items-center gap-1.5 font-mono text-xs text-gray-500">
                    ...
                    <span className="text-gray-500">{item.to_value}</span>
                  </div>
                )}
                {item.reason && (
                ...
                {item.note && (
                  <p className="font-sans text-xs text-gray-500 italic">{item.note}</p>
                )}
              </div>
            ))
          )}
...
function DealCard({
  ...
  return (
    <div className="bg-gray-100/60 border border-gray-200/60 hover:bg-gray-100 hover:border-gray-300/80 rounded transition-colors">
      {/* Main row */}
      <div className="flex items-start gap-3 px-3 py-2.5">
        ...
        <div className="hidden sm:flex items-center gap-4 shrink-0">
          ...
          <div className="text-right min-w-[65px]">
            <div className="font-sans text-[10px] text-gray-500 uppercase tracking-wide">Mo. Net</div>
          ...
          <div className="hidden lg:block min-w-[95px]">
            <div className="font-sans text-[10px] text-gray-500 uppercase tracking-wide mb-1">Score</div>
            <OverallScoreBar score={deal.overall_score ?? 0} />
          </div>

          {/* Post-HELOC */}
          <div className="hidden xl:block text-right min-w-[70px]">
            <div className="font-sans text-[10px] text-gray-500 uppercase tracking-wide">Post-HELOC</div>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            onClick={onHistory}
            className="p-1.5 text-gray-500 hover:text-cyan-400 transition-colors rounded"
            title="View history"
          >
          ...
          {deal.buy_vs_build_reason && (
            <button
              ...
              className="p-1.5 text-gray-500 hover:text-amber-400 transition-colors rounded text-[10px] font-mono"
      ...
      {expanded && deal.buy_vs_build_reason && (
        <div className="px-3 pb-2.5 font-sans text-xs text-gray-500 bg-gray-50/50 border-t border-gray-200">
      ...
      <div className="mx-2 my-3 flex flex-col items-center justify-center py-8 gap-3 bg-red-500/5 border border-red-500/15 rounded-lg">
        ...
        <div className="text-center">
          ...
          <div className="font-sans text-xs text-gray-500 mt-1">
            Start it: <span className="font-mono text-gray-500">cd ~/Eva/modules/deal-scout && python main.py</span>
          </div>
        </div>
        <button
          onClick={onRefresh}
  ...
  if (status === 'loading') {
    return (
      <div className="flex flex-col gap-2 py-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="h-14 bg-gray-100/40 rounded border border-gray-200/60 animate-pulse" />
        ))}
      </div>
    );
  }
