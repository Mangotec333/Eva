"""
EVA Video Generator — state-ledger emitter (behind a Protocol).

Video lifecycle events (created/storyboard/approved/rendering/rendered/failed)
are appended to the governed Eva State Ledger (``modules/eva-state``, port 8769)
so the whole system shares one timeline. Adapted from
``modules/media-editor/state_client.py``: the write is behind a
``StateLedgerClient`` Protocol so tests and the sandbox never touch the network,
and any failure is an honest ``{"ok": False}`` rather than a faked success — the
render loop still succeeds locally even if the ledger is down.

Set ``EVA_VIDEO_OFFLINE=1`` to force the stub emitter.
"""
...
SOURCE_SURFACE = "video-generator"
