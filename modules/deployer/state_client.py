"""
EVA Deployer — state-ledger emitter (behind a Protocol).

Every self-deploy decision the CI/CD agent makes — a clean fast-forward + service
restart, an aborted conflict, or an outright failure — is written back to the
governed Eva State Ledger (``modules/eva-state``, port 8769) so the whole system
shares one timeline and Eva can see when it last upgraded itself. This module is
the single seam for that write, behind a ``StateLedgerClient`` Protocol so tests
and the sandbox never touch the network.

Mirrors ``modules/triage-brain/state_client.py`` and
``modules/social-scheduler/state_client.py`` (same shape, same honest-failure
contract) — only ``SOURCE_SURFACE`` / ``track`` differ.
"""
