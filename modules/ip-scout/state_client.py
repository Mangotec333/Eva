"""
EVA IP-Scout — eva-state ledger client (emit + read, behind a Protocol).

IP-Scout logs its triage actions — idea seeded, scan run, disclosure created,
report written — back to the governed Eva State Ledger (``modules/eva-state``,
port 8769) so the whole system shares one timeline. It also READS eva-state
events to mine Eva's own activity for repeatable/novel process patterns that
could be invention disclosures (see ``mining.py``).

Mirrors ``modules/brand-builder/state_client.py`` (same emit shape, same honest-
failure contract) and adds a ``read_events`` method for activity mining. Stdlib
only (urllib), so tests / the sandbox never touch the network.
"""
...
class StubStateLedgerClient:
    ...
    seed events for activity mining. No network."""
...
class HttpStateLedgerClient:
    """Live client — POSTs to /events, GETs /events for mining."""
    ...
    def emit(self, *, event_type: str, summary: str = "",
        ...
        body = {
            ...
            "project": PROJECT,
            "track": "research",
            "entity_type": "invention_disclosure",
            "entity_id": entity_id,
            "payload": payload or {},
        }
        import urllib.error
    ...
    def read_events(self, *, since: Optional[str] = None,
        ...
        except Exception:  # ledger down — mining just yields nothing
            return []
...
def build_state_client(offline: Optional[bool] = None,
    ...
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_IP_OFFLINE") == "1"
    if use_stub:
