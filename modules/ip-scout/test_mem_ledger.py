"""
EVA IP-Scout — offline memory + ledger test (stdlib only, zero network).
...
Run: python modules/ip-scout/test_mem_ledger.py
"""
...
def test_memory_and_ledger() -> None:
    os.environ["EVA_IP_DIR"] = tempfile.mkdtemp()
    import store
    store.set_memory("greeting", "hello")
    assert store.get_memory("greeting") == "hello"
    assert store.get_memory("missing", "dflt") == "dflt"
    store.append_ledger("created", entity_type="thing", entity_id="1",
                        details={"n": 1})
    rows = store.query_ledger()
    assert len(rows) == 1 and rows[0]["details"] == {"n": 1}
    _assert_ledger_immutable(str(store.db_path()))
