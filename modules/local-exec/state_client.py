"""
EVA Local-Exec — state-ledger emitter (behind a Protocol).

Every shell run the "Mac hands" layer performs — an allowlisted auto-run, an
approved run, a blocked/pending gate, a failure, or an approval timeout — is
written back to the governed Eva State Ledger (``modules/eva-state``, port 8769)
so the whole system shares one timeline and Eva can audit exactly what it ran on
the Mac. This module is the single seam for that write, behind a
``StateLedgerClient`` Protocol so tests and the sandbox never touch the network.

Mirrors ``modules/deployer/state_client.py`` and
