"""
EVA Activity-Tracker-Agent — eva-state ledger client (emit + read, behind a
Protocol). Mirrors ``modules/idea-generator-agent/state_client.py`` exactly
(same emit shape, same honest-failure contract, same read_events surface) —
this is the seam that makes the daily digest see the shared timeline every
other lobe already writes to, and lets Diracatron see this agent's findings
back.
"""
...
SOURCE_SURFACE = "activity-tracker-agent"
TRACK = "ops"
...
class StubStateLedgerClient:
    """Offline client — records emitted events in memory, serves a fixed set
    of seed events for the daily digest. No network."""
...
class HttpStateLedgerClient:
    """Live client — POSTs to /events, GETs /events for the daily digest."""
    ...
    def read_events(self, *, since: Optional[str] = None,
        ...
        except Exception:  # ledger down — digest just reports a gap
            return []
...
def build_state_client(offline: Optional[bool] = None,
    ...
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_ACTIVITY_OFFLINE") == "1"
    if use_stub:
