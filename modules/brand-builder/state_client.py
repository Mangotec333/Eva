"""
EVA Brand-Builder — eva-state ledger emitter (behind a Protocol).

The Brand Builder logs its strategic actions — plan generated, brief created,
blueprint parsed/stale — back to the governed Eva State Ledger
(``modules/eva-state``, port 8769) so the whole system shares one timeline, and
so content-engine can pick up ``brand_brief_created`` events.

Mirrors ``modules/social-scheduler/state_client.py`` exactly (same shape, same
...
def build_state_client(offline: Optional[bool] = None) -> StateLedgerClient:
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_BRAND_OFFLINE") == "1"
