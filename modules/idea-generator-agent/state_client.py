"""
EVA Idea-Generator-Agent — eva-state ledger client (emit + read, behind a
Protocol). Mirrors ``modules/ip-scout/state_client.py`` exactly (same emit
shape, same honest-failure contract, same read_events surface). This is the
single seam that makes every other EVA lobe (Diracatron included) see this
agent's findings on the shared timeline — no separate sync mechanism needed.
"""
...
SOURCE_SURFACE = "idea-generator-agent"
TRACK = "strategy"
...
class StubStateLedgerClient:
    ...
    of seed events for the alignment digest. No network."""
...
class HttpStateLedgerClient:
    """Live client — POSTs to /events, GETs /events for the alignment digest."""
...
def build_state_client(offline: Optional[bool] = None,
    ...
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_IDEA_OFFLINE") == "1"
    if use_stub:
