"""
EVA Diracatron — state-ledger emitter (behind a Protocol)
=========================================================

Every triage decision Diracatron makes is written back to the governed Eva
State Ledger (``modules/eva-state``, port 8769) so the whole system shares one
timeline and Eva *learns* from what the brain dispatched. This module is the
single seam for that write, behind a ``StateLedgerClient`` Protocol so tests
and the sandbox never touch the network.

Mirrors ``modules/ghl-agent/state_client.py`` (same shape, same honest-failure
contract) — only the ``SOURCE_SURFACE`` differs.
...
- ``HttpStateLedgerClient`` — POSTs to ``http://localhost:8769/events``. On any
  ...
  a triage pass still succeeds locally even if the ledger is down.
...
def build_state_client(offline: Optional[bool] = None) -> StateLedgerClient:
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_DIRACATRON_OFFLINE") == "1"
