"""
EVA GHL Agent — state-ledger emitter (behind a Protocol)
========================================================

Lead lifecycle events are written to the governed Eva State Ledger
(``modules/eva-state``, port 8769) so the whole system shares one timeline. This
module is the single seam for that write, behind a ``StateLedgerClient``
...
- ``HttpStateLedgerClient`` — POSTs to ``http://localhost:8769/events``. On any
  ...
  the capture loop still succeeds locally even if the ledger is down.
- ``StubStateLedgerClient`` — records emitted events in memory for tests.

Every emission carries ``source_surface="ghl-agent"`` and
``project="Eva Acquisition"`` so the ledger's project views group them.
"""
...
def build_state_client(offline: Optional[bool] = None) -> StateLedgerClient:
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_GHL_OFFLINE") == "1"
