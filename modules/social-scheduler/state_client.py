"""
EVA Social-Scheduler — state-ledger emitter (behind a Protocol).

The daily publisher logs every published post + engagement-sync summary back to
the governed Eva State Ledger (``modules/eva-state``, port 8769) so the whole
system shares one timeline. This is the single seam for that write, behind a
...
``modules/finance-tracker/state_client.py`` (same shape, same honest-failure
...
SOURCE_SURFACE = "social-scheduler"
...
def build_state_client(offline: Optional[bool] = None) -> StateLedgerClient:
    use_stub = offline
    if use_stub is None:
        use_stub = os.environ.get("EVA_SOCIAL_SCHEDULER_OFFLINE") == "1"
