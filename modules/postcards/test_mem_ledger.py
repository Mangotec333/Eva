"""
EVA Postcards — offline memory + ledger test (stdlib only, zero network).
...
Run: python modules/postcards/test_mem_ledger.py
"""
...
def _assert_ledger_immutable(db_path: str) -> None:
    ...
    conn.execute(
        "INSERT INTO publish_ledger (id, ts, event_type) VALUES ('seed','t','ev')")
    conn.commit()
    blocked_update = blocked_delete = False
    try:
        conn.execute("UPDATE publish_ledger SET actor='x'")
    except sqlite3.Error as e:
        blocked_update = "append-only" in str(e)
    try:
        conn.execute("DELETE FROM publish_ledger")
    except sqlite3.Error as e:
