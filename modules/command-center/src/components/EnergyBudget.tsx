    ...
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-sans text-xs text-gray-400">{bucket.label}</span>
        </div>
        <span className="font-mono text-xs font-bold text-gray-300 tabular-nums">
          {bucket.percentage}%
        </span>
      </div>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-1">
          {ENERGY_BUCKETS.map(b => (
            <div key={b.label} className="flex items-center justify-between">
              <span className="font-sans text-[10px] text-gray-600 truncate">{b.label}</span>
