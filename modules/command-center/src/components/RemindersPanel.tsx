...
function dueBadgeClass(due_at: string, done: boolean): string {
  if (done) return 'text-gray-600 bg-gray-800 border-gray-700';
...
function AddForm({ onSave, onCancel }: AddFormProps) {
  ...
  return (
    <div className="mt-3 bg-gray-950 border border-cyan-400/20 rounded-lg p-3 space-y-2">
      <input
        ref={titleRef}
        type="text"
        ...
        onKeyDown={handleKey}
        className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-cyan-400/50 font-mono"
      />
      <div className="flex gap-2">
        <input
          ...
          onKeyDown={handleKey}
          className="flex-1 bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 focus:outline-none focus:border-cyan-400/50 font-mono [color-scheme:dark]"
        />
        <input
          type="text"
          ...
          onKeyDown={handleKey}
          className="w-32 bg-gray-900 border border-gray-700 rounded px-3 py-1.5 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-cyan-400/50 font-mono"
        />
      </div>
      <div className="flex gap-2 justify-end">
        <button
          onClick={onCancel}
          className="px-3 py-1 text-xs font-mono text-gray-400 hover:text-gray-200 border border-gray-700 rounded hover:border-gray-500 transition-colors"
        >
          CANCEL
        </button>
        <button
...
function CalendarCard({ event }: { event: CalendarEvent }) {
  return (
    <div className="flex-shrink-0 w-40 bg-gray-950 border border-gray-800 rounded-lg p-2.5 space-y-1">
      ...
      <p className="text-xs text-gray-300 leading-tight line-clamp-2">{event.title}</p>
    </div>
  );
}
...
function ReminderRow({ reminder, onToggle, onDelete }: ReminderRowProps) {
  ...
  return (
    <div
      className={`flex items-center gap-2.5 py-2 px-1 border-b border-gray-800/50 last:border-0 group transition-opacity ${
    ...
    >
      ...
      <button
        ...
        className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity text-gray-600 hover:text-red-400"
...
function DueAlert({ reminder, onDismiss }: DueAlertProps) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 bg-gray-900 border border-cyan-400/50 rounded-lg p-4 shadow-lg shadow-cyan-400/10 animate-pulse w-72 max-w-[calc(100vw-2rem)]">
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-200 leading-tight truncate">{reminder.title}</p>
...
export default function RemindersPanel() {
  ...
  return (
    <>
      {/* ── Panel ─────────────────────────────────────────────────────────── */}
      <div className="bg-gray-900 border border-gray-800 rounded-lg flex flex-col h-full min-h-0">
          ...
          <div className="px-4 pt-3 pb-1 flex-shrink-0">
            ...
            {calendarLoading ? (
              <div className="flex items-center gap-2 py-2">
                ...
                <span className="text-xs text-gray-600 font-mono">Loading calendar…</span>
              </div>
            ) : calendarEvents.length === 0 ? (
              <div className="py-2 text-[11px] text-gray-600 font-mono">
          ...
          <div className="px-4 pt-2 pb-3">
            ...
            {loading ? (
              <div className="flex items-center gap-2 py-3">
                <Loader2 size={12} className="animate-spin text-gray-600" />
                <span className="text-xs text-gray-600 font-mono">Loading reminders…</span>
              </div>
            ) : reminders.length === 0 ? (
              ...
              <div>
                ...
                {doneReminders.length > 0 && (
                  <details className="mt-2 group">
                    <summary className="font-mono text-[10px] text-gray-600 hover:text-gray-400 cursor-pointer select-none list-none flex items-center gap-1 py-1">
                      <span className="border border-gray-700 rounded px-1.5 py-px bg-gray-800/50 hover:bg-gray-700/50 transition-colors">
