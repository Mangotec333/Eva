  ...
  return (
    <span className={`px-2 py-0.5 rounded border font-mono text-[10px] font-semibold tabular-nums ${
      ...
      near  ? 'bg-amber-500/15 text-amber-400 border-amber-500/30' :
              'bg-gray-700/30 text-gray-500 border-gray-700/40'
    }`}>
      {count} chars
    </span>
  );
    ...
    <ModalOverlay onClose={onCancel}>
      <div className="w-full max-w-lg bg-gray-900 border border-gray-700 rounded-xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 shrink-0">
          ...
          <button onClick={onCancel} className="p-1 text-gray-600 hover:text-gray-300 rounded transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {result ? (
            ...
            <>
              ...
              <div>
                ...
                <div className="bg-gray-950 border border-gray-800 rounded-lg px-4 py-3 font-sans text-sm text-gray-300 leading-relaxed whitespace-pre-wrap max-h-56 overflow-y-auto">
                  {post.main_post}
                </div>
              </div>
              ...
              {post.comment_post && (
                <div>
                  ...
                  <div className="bg-gray-950 border border-gray-800 rounded-lg px-4 py-3 font-sans text-sm text-gray-400 leading-relaxed whitespace-pre-wrap max-h-36 overflow-y-auto">
                    {post.comment_post}
                  </div>
                </div>
              )}
        ...
        {/* Footer */}
        <div className="px-5 py-4 border-t border-gray-800 flex justify-end gap-2 shrink-0">
          {result?.success ? (
            <button
              onClick={onCancel}
              className="px-4 py-2 bg-gray-800 border border-gray-700 text-gray-300 rounded font-mono text-xs font-bold hover:bg-gray-700 transition-colors"
            >
              Close
            </button>
          ) : result?.success === false ? (
            <>
              <button onClick={onCancel} className="px-3 py-1.5 font-mono text-xs text-gray-500 hover:text-gray-300 border border-gray-700 rounded transition-colors">Cancel</button>
              <button
                onClick={handlePost}
                disabled={posting}
...
function GeneratePanel({
  ...
  return (
    <div className="bg-gray-800/60 border border-gray-700/60 rounded-lg p-4 flex flex-col gap-4">
      {/* Panel header */}
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-gray-950 border border-gray-700 rounded px-3 py-2 font-sans text-sm text-gray-200 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/60 transition-colors"
    ...
    <div className="flex flex-col gap-3 pt-3 border-t border-gray-800">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          ...
          <span className={`font-mono text-[10px] tabular-nums font-semibold ${charOver ? 'text-red-400' : charCount > 1800 ? 'text-amber-400' : 'text-gray-600'}`}>
            {charCount} / 2000
          </span>
        </div>
        <textarea
          className={`w-full bg-gray-950 border rounded px-3 py-2.5 font-sans text-sm text-gray-200 resize-none focus:outline-none transition-colors placeholder:text-gray-600 ${
            charOver ? 'border-red-500/60 focus:border-red-500' : 'border-gray-700 focus:border-cyan-500/60'
          }`}
          rows={8}
          value={mainPost}
      ...
      <div>
        ...
        <textarea
          className="w-full bg-gray-950 border border-gray-700 rounded px-3 py-2.5 font-sans text-sm text-gray-200 resize-none focus:outline-none focus:border-cyan-500/60 transition-colors placeholder:text-gray-600"
          rows={3}
          value={commentPost}
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={onCancel}
          className="px-3 py-1.5 bg-gray-800 border border-gray-700 text-gray-400 rounded font-mono text-xs hover:bg-gray-700 hover:text-gray-200 transition-colors"
        >
          Cancel
        </button>
      </div>
...
function PostCard({
  ...
  return (
    <div className="bg-gray-800/60 border border-gray-700/60 hover:border-gray-600/80 rounded-lg transition-colors">
      <div className="px-4 py-3 flex flex-col gap-2.5">
        ...
        {/* Action row */}
        {!editing && (
          <div className="flex items-center gap-1.5 flex-wrap pt-1 border-t border-gray-800">
            {/* Edit */}
            <button
            ...
            <button
              ...
              disabled={deleting}
              className="flex items-center gap-1 px-2 py-1.5 text-gray-600 hover:text-red-400 rounded font-mono text-[11px] disabled:opacity-50 transition-colors ml-auto"
              title="Delete post"
            >
...
function EmptyState({ isOnline, onRefresh }: { isOnline: boolean; onRefresh: () => void }) {
      ...
      <div className="mx-2 my-3 flex flex-col items-center justify-center py-10 gap-3 bg-red-500/5 border border-red-500/15 rounded-lg">
        ...
        <div className="text-center">
          ...
          <div className="font-sans text-xs text-gray-500 mt-1">
            Start it:{' '}
            <span className="font-mono text-gray-400">cd ~/Eva/modules/content-engine && python main.py</span>
          </div>
        </div>
        <button
          onClick={onRefresh}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 gap-2">
      ...
      <div className="font-mono text-[10px] text-gray-600">Use Generate Post to create new LinkedIn content</div>
    </div>
  );
}
...
function LoadingSkeleton() {
  return (
    <div className="flex flex-col gap-2 py-2">
      {[1, 2, 3].map(i => (
        <div key={i} className="h-20 bg-gray-800/40 rounded-lg border border-gray-800/60 animate-pulse" />
      ))}
    </div>
  );
}
          ...
          <div className="flex items-center gap-2">
            ...
            <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded border font-mono text-[10px] font-semibold ${
              linkedInConnected
                ...
                : 'bg-gray-700/30 border-gray-700/40 text-gray-500'
            }`}>
